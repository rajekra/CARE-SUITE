	...
	public static void main(String args[]) throws Exception {
		SparkSession sparkSession = SparkSession.builder().master(CommonConstants.SPARK_MASTER)
				...
				//  .config("spark.mongodb.input.database", "icare")
				  .getOrCreate();
		new CopyOfInpatientMdcwisePrediction(sparkSession);
	}
...
//				}
		...
		StringIndexer admtDiagCdIndexer = null;
		OneHotEncoder admtDiagCdVec = null;
				...
				{		
					...
					admtDiagCdIndexer = new StringIndexer().setInputCol("admtDiagCd").setOutputCol("admtDiagCdIndexer");
					admtDiagCdIndexer.setHandleInvalid("keep");
					admtDiagCdVec = new OneHotEncoder().setInputCol("admtDiagCdIndexer").setOutputCol("admtDiagCdVec");
...
//					}
//				}
				StringIndexer prncplDgnsCdIndexer = null;
				OneHotEncoder prncplDgnsCdVec = null;
				...
				{		
					...
					if(null!=d1Max && !d1Max.isEmpty() && d1Max.size()>1 && null!=d1Max.get(1).getAs("prncplDgnsCd"))
					{
						...
						prncplDgnsCdIndexer = new StringIndexer().setInputCol("prncplDgnsCd").setOutputCol("prncplDgnsCdIndexer");
						prncplDgnsCdVec = new OneHotEncoder().setInputCol("prncplDgnsCdIndexer").setOutputCol("prncplDgnsCdVec");
				...
				//		assemblers.add("prncplDgnsCd");
					...
					else
					{
						...
						System.out.println("ELSE prncplDgnsCd");
					}
				}
...
//				String[] STRING_ARRAY = {"admtDiagCdIndexer","admtDiagCdVec","prncplDgnsCdIndexer","prncplDgnsCdVec","patientGenderIndexer","patientGenderVec","drgCodeBucketizer","mdcBucketizer","los"};
//				VectorAssembler assembler = new VectorAssembler().setInputCols(STRING_ARRAY).setOutputCol("features_temp");
//				Normalizer normalizer = new Normalizer().setInputCol("features_temp").setOutputCol("features");
//				
//				PipelineStage[] stages = {	admtDiagCdIndexer,admtDiagCdVec,prncplDgnsCdIndexer,prncplDgnsCdVec,patientGenderIndexer,patientGenderVec,drgCodeBucketizer,mdcBucketizer,assembler,normalizer,linearRegression};
//				Pipeline pipeLine = new Pipeline().setStages(stages);
		...
		trainValidationSplit.setSeed(1234);
